...
import { SimpleVoxelGrid } from "./scene-components/simple-voxel-grid"

// Scene Four: Voxel Sigil Interface
const SceneFour = ({ scale = 1, opacity = 1, visible = true }) => {
  ...
  return (
    <group ref={groupRef} scale={typeof scale === "number" ? [scale, scale, scale] : scale} visible={visible}>
      <SimpleVoxelGrid time={time} opacity={opacity} />
      <Text
        position={[0, -1.8, 0]}
        fontSize={0.2}
