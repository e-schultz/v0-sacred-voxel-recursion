...
import { SimpleGradient } from "./scene-components/simple-gradient"

// Scene Three: Oscillating Gradients
const SceneThree = ({ scale = 1, opacity = 1, visible = true }) => {
  ...
  return (
    <group ref={groupRef} scale={typeof scale === "number" ? [scale, scale, scale] : scale} visible={visible}>
      <SimpleGradient time={time} opacity={opacity} />
      <Text
        position={[0, -1.8, 0]}
        fontSize={0.2}
...
export default memo(SceneThree)
